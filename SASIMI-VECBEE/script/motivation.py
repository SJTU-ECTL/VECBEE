...
# approximate
with open('./result/motivation/motivation-1024-samples.log') as f:
    lines = f.readlines()
...
for line in lines:
    ...
    if line.find('run ') != -1:
        ...
        rra.append((1 - float(line[7:]) / 2759.0) * 100.0)
        if currEr > 0.01:
            ers.append(er)
            rras.append(rra)
...
# print(indexMax)
# print(indexMed)
lines = plt.plot(ers[indexMin], rras[indexMin], marker=mk[0], markersize=10, markeredgewidth=2.0, fillstyle='none', label='w/o high accuracy\nerror estimation$-$ worst')
lines[0].set_linewidth(2.0)
plt.text(ers[indexMin][-2] - 0.15, rras[indexMin][-2] - 0.8, '(' + '%.2f,' % (ers[indexMin][-2]) + '%.2f' % (rras[indexMin][-2]) + ')', fontsize=ftSize)
lines = plt.plot(ers[indexMax], rras[indexMax], marker=mk[1], markersize=10, markeredgewidth=2.0, fillstyle='none', label='w/o high accuracy\nerror estimation$-$ best')
lines[0].set_linewidth(2.0)
...
lines = plt.plot(ers[indexMed], rras[indexMed], marker=mk[2], markersize=10, markeredgewidth=2.0, fillstyle='none', label='w/o high accuracy\nerror estimation$-$ median')
lines[0].set_linewidth(2.0)
plt.text(ers[indexMed][-2] - 0.20, rras[indexMed][-2] - 0.8, '(' + '%.2f,' % (ers[indexMed][-2]) + '%.2f' % (rras[indexMed][-2]) + ')', fontsize=ftSize)
...
plt.legend(loc='lower right', bbox_to_anchor=(1.10, 0), fontsize=10)
# plt.show()
