...
plt.rcParams['figure.figsize'] = (6.0, 2.9)
...
# ])
# lines = plt.plot(er, rra, color='r', marker='o', markersize=8, markeredgewidth=1.0, fillstyle='none', label='with high accuracy\nerror estimation')
# lines[0].set_linewidth(2.0)
# plt.text(er[-1] + 0.01, rra[-1] - 0.8, '(0.99, 12.65)', fontsize=ftSize)

# dashed line
plt.plot([1, 1], [-1, 15], linewidth=1.0, linestyle='--', color='k')
...
# print(indexMax)
# print(indexMed)
lines = plt.plot(ers[indexMin], rras[indexMin], marker=mk[0], markersize=8, markeredgewidth=1.0, fillstyle='none', label='with accurate$-$worst')
...
lines = plt.plot(ers[indexMax], rras[indexMax], marker=mk[1], markersize=8, markeredgewidth=1.0, fillstyle='none', label='with accurate$-$best')
lines[0].set_linewidth(1.0)
...
lines = plt.plot(ers[indexMed], rras[indexMed], marker=mk[2], markersize=8, markeredgewidth=1.0, fillstyle='none', label='with accurate$-$median')
lines[0].set_linewidth(1.0)
# print(ers[indexMed], rras[indexMed])
plt.text(ers[indexMed][-1] - 0.17, rras[indexMed][-1] - 2.8, 'median,\n(' + '%.2f,' % (ers[indexMed][-1]) + '%.2f' % (rras[indexMed][-1]) + ')', fontsize=ftSize)
...
indexMax = np.argwhere(allRra == allRra.max())[0][0]
indexMed = np.argwhere(allRra == np.median(allRra))[0][0]
lines = plt.plot(ers[indexMin], rras[indexMin], marker=mk[3], markersize=8, markeredgewidth=1.0, fillstyle='none', label='without accurate$-$worst')
lines[0].set_linewidth(1.0)

lines = plt.plot(ers[indexMax], rras[indexMax], marker=mk[4], markersize=8, markeredgewidth=1.0, fillstyle='none', label='without accurate$-$best')
lines[0].set_linewidth(1.0)

lines = plt.plot(ers[indexMed], rras[indexMed], marker=mk[5], markersize=8, markeredgewidth=1.0, fillstyle='none', label='without accurate$-$median')
lines[0].set_linewidth(1.0)
plt.text(ers[indexMed][-2] - 0.15, rras[indexMed][-2] - 2.8, 'median,\n(' + '%.2f,' % (ers[indexMed][-2]) + '%.2f' % (rras[indexMed][-2]) + ')', fontsize=ftSize)
plt.text(ers[indexMed][-1] - 0.50, rras[indexMed][-1] - 2.8, 'median,\n(' + '%.2f,' % (ers[indexMed][-1]) + '%.2f' % (rras[indexMed][-1]) + ')', fontsize=ftSize)
